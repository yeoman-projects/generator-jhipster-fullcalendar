...
export class CalendarComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.calendarService
        ...
        .subscribe(
          (res: HttpResponse<ICalendar[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInCalendars(): void {
    this.eventSubscriber = this.eventManager.subscribe('calendarListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICalendar[] | null, headers: HttpHeaders, page: number): void {
